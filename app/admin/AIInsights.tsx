...
- outlook: positive if avgScore >= 55, concerning if avgScore < 40, neutral otherwise`
  ...
  const outlookConfig = {
    positive:   { label: 'Positive Outlook',   Icon: TrendingUp,   color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' },
    neutral:    { label: 'Neutral Outlook',     Icon: Minus,        color: 'text-amber-400',   bg: 'bg-amber-500/10',   border: 'border-amber-500/20'   },
    concerning: { label: 'Concerning Outlook',  Icon: TrendingDown, color: 'text-red-400',     bg: 'bg-red-500/10',     border: 'border-red-500/20'     },
  ...
  return (
    <div className="space-y-4 mb-8">
      ...
      <div className="flex items-center gap-3">
        <div className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold uppercase tracking-widest px-3.5 py-1.5 rounded-full">
          <Lightbulb className="w-3 h-3" />
          AI Insights
        </div>
        <span className="text-slate-600 text-xs">Generated by GPT-4o mini · Based on {submissions.length} submission{submissions.length !== 1 ? 's' : ''}</span>
      </div>

      {/* Summary card */}
      <div className="bg-brand-navy-mid border border-white/8 rounded-xl p-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-brand-navy-mid border border-white/8 rounded-xl overflow-hidden">
          <div className="flex items-center gap-2.5 px-5 py-4 border-b border-white/8">
            ...
            <span className="text-white text-sm font-semibold">Top Risk Patterns</span>
          </div>
          <div className="px-5 py-4 space-y-3.5">
            {insights.topRisks?.map((risk, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className="w-5 h-5 shrink-0 rounded-full bg-red-500/15 border border-red-500/25 flex items-center justify-center text-red-400 text-[10px] font-bold mt-0.5">
                  {i + 1}
                </span>
                <p className="text-slate-300 text-[13px] leading-relaxed">{risk}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-brand-navy-mid border border-white/8 rounded-xl overflow-hidden">
          ...
          <div className="px-5 py-4 space-y-3.5">
            {insights.recommendations?.map((rec, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className="w-5 h-5 shrink-0 rounded-full bg-blue-500/15 border border-blue-500/25 flex items-center justify-center text-blue-400 text-[10px] font-bold mt-0.5">
                  {i + 1}
                </span>
                <p className="text-slate-300 text-[13px] leading-relaxed">{rec}</p>
              </div>
            ))}
          </div>
        </div>
